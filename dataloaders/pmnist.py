...
def get(data_path, args, seed=0, fixed_order=False, pc_valid=0.1, samples_per_task=-1):
    data = {}
    output_info = []
    size = [1, 28, 28]
    ...
    for t in data.keys():
        ...
        if samples_per_task > 0:
            nvalid = int(pc_valid * samples_per_task)
            ivalid = torch.LongTensor(r[:nvalid])
            itrain = torch.LongTensor(r[nvalid:samples_per_task])
        else:
    ...
    return data, output_info, n_inputs, nperm, n_outputs